...
This requires the `Storage` port to be able to say what it did. `save(): Promise<void>` was a validator: it reports *threw / didn't throw*, and "didn't throw" does not mean the bytes arrived. The port is now `Promise<WriteReceipt | void>` (`src/storage/writeOutcome.ts`) — an assignable widening, so an unmigrated adapter still compiles, with `void` read as "presumed landing" (the old behaviour). `local`, `github`, `gitlab`, `gdrive` and `pcloud` are migrated: each now reports `landed()`/`skipped(Coalesced)` on success and throws a `ClassifiedWriteError` (a `WriteFailureKind`) on failure instead of a bare `Error`, so `parseWriteFailure()` (`src/storage/parse.ts`) has a real signal to classify — `dropbox`, `webdav`, `s3`, `sharepoint` and `onedrive` still resolve `void`, read as Proven until they're migrated too.
...
5. ~~**Write outcome**~~ — `WriteReceipt`, `PersistHealth`, adapter migration one at a time — done (§3.2/§3.3). Non-leader diffusion of a broken leader's health across same-browser tabs (a `BroadcastChannel`, mirroring the leader-election scoping) and the remaining five adapters (`dropbox`, `webdav`, `s3`, `sharepoint`, `onedrive`) are left for a follow-up — they still resolve `void`/read as Proven in the meantime, never locking on ignorance.
