...
> are not; §6.1's presence probe is wired for the hub transport only, with its
> P2P path remaining unimplemented).
...
`Export a copy` (`src/ui/ExportDialog.svelte` + `ExportFormats.svelte`) is the genuinely new UI surface the contract required — all four portable codecs (text/markdown/html/json) already existed, so the format falls out of the chosen extension; no codec was written. It reaches the shared `Y.Doc` through `src/editor/exportBridge.svelte.ts` (same module-level-bridge shape as the room-name bridge), bound whenever the Editor is mounted — including while write-gated, since export is a read. Reachable from three places: the read-only band itself (`SyncBanner`'s waiting tier, next to Copy invite link / Connect storage), Settings, and an Export button in the header capsule (mobile nav dock and desktop capsule alike) — so it works whether or not the gate has armed, and doesn't require opening Settings to find. Import sits beside it in the same header for the same reason, disabled whenever `writeLocked` holds — it's a write, not a read, so it doesn't get the same "always reachable" guarantee. A fifth entry, `PDF (print)`, sits alongside the four codec buttons: not a codec (nothing is encoded ahead of time), it triggers the browser's native print dialog against a dedicated `src/styles/print.css` stylesheet — "Save as PDF" gives real, selectable text with zero added dependency. A sixth, `Word (.docx)`, is a one-way `ExportCodec` (`src/format/types.ts`) rather than a `Codec` — never a backend save target, so it has no `decode` — that walks the ProseMirror tree into the `docx` package's declarative API (`src/format/docxEncode.ts`), loaded via a dynamic `import()` so its ~100kB gzipped cost lands only on a page that actually triggers a Word export.

Also active while read-only: text selection and copy, scrolling and outline, the command palette's navigation (jumping to a heading, reaching an action, opening another document — all reads), Share, Settings, connecting a backend, theme and identity, loading from the backend. Inactive: typing, formatting toolbar (visible but disabled — removing it would suggest a different app), slash menu, input rules, undo/redo, renaming the document (the title lives in the shared doc, so renaming is a collaborative write). The palette's `/` insert rows go with the slash menu, and are withheld rather than shown dead: they are the same commands, and a row that cannot run is not a disabled control the way a toolbar button is.
...
Resolved: `newRoomId()` (`src/collaboration/roomId.ts`) draws room ids from `crypto.randomUUID()`, and `browserId()` (`src/collaboration/browserId.ts`) mints the same way (with a `crypto.getRandomValues` fallback). **New rooms are encrypted by default** — `newRoom()` mints a secret-link key (`#k=`) alongside the id, so a freshly created room is end-to-end encrypted from the start; a bare first visit (no `?room=`, no `VITE_DEFAULT_ROOM`) mints the same way rather than pooling every stranger into one shared landing room, where the first thing a visitor meets is somebody else's document (`resolveLandingRoom()`, `src/collaboration/config.ts`); the Share dialog surfaces this as the 🔒 *Encrypted* badge, so the choice is visible rather than implicit. The README states plainly what the room id protects and what it doesn't. CSPRNG room ids and encryption by default were prerequisites of the probe, not a later chore — that precondition is now satisfied.
...
**Hub: wired.** `probeWebsocketPresence()` (`src/collaboration/presenceProbe.ts`) opens a raw `WebSocket` to the room, exactly like `websocketCollab`'s URL construction, but never constructs a `Y.Doc` and never sends a byte itself. `@y/websocket-server`'s `setupWSConnection` pushes two messages to every new connection unprompted: a sync-step-1 (a state *vector*, not content) and, only if peers are already present, their current awareness states. The probe decodes only the second message type: the sync frame is never parsed, so document content, encrypted or not, never reaches the probe. It reports `HallPresence`: `unknown` while connecting, `empty` once a short settle window (`PRESENCE_PROBE_SETTLE_MS`) passes with nothing pushed, `someone` (with a `lastSeen` `EpochMs`) once a non-null awareness state arrives, and it keeps listening afterward, so a later arrival or the ~31s server-side awareness timeout on a departure both update it live. Wired into `MeetingJoinDialog.svelte`: pasting a meeting link fires a non-blocking probe of the derived room and toasts "Someone's already in there" / "Looks empty in there right now" once it resolves, without delaying the join tab. The hub-memory cost this section already documents is unmitigated by design (see above); the probe's own `destroy`/`stop()` closes its socket promptly, but the server-side `Y.Doc` it caused to be allocated is not freed by that.

`MeetingJoinDialog` opens the join tab into the very room it just started probing, on the same hub, so on a fast connection the new tab's own `WebsocketProvider` publishes its awareness state before the probe's settle window elapses, and the still-listening probe socket receives that self-join relayed back and misreads it as a peer (a false `someone` on a genuinely empty room, not rare on a fast/warm connection). Fixed without adding delay: `mintSelfProbeMarker()` (`src/collaboration/selfProbeMarker.ts`) mints a single-use `SelfProbeMarker` in `MeetingJoinDialog` before either the probe starts or the new tab opens; it is passed to `probeWebsocketPresence()` as `selfMarker` and into the new tab via a `?selfProbe=` URL param, which `App.svelte` parses (the URL-param IO boundary) and `Editor.svelte` includes in the `PeerAwarenessState` (`selfProbeMarker` field, parsed by `parsePeerAwarenessState()`) it broadcasts once connected. The probe discards any awareness entry carrying its own marker before counting peers, so the new tab's self-announcement is recognized and excluded rather than read as a stranger, while a real second tab of the same browser (§7, "A second tab of your own browser"), which carries a stable `BrowserId` but never this one-shot marker, still counts as `someone`, matching that section's rule. Nothing waits longer: the exclusion is a same-tick filter on data the probe already receives, not an extra round-trip.

**P2P: investigated, not implementable as a general probe.** The obvious retry of the hub's trick (subscribe to the room's topic, forge a discovery message, watch a real peer's own `Room` react, disconnect before anything completes) was investigated directly against `y-webrtc@10.3.0`'s and `y-webrtc-signaling`'s (the same package's `bin/server.js`) actual source, not re-argued from the original vague reasoning. The signaling server itself is *not* the wall: its `'publish'` handler relays to every topic subscriber unconditionally (`topics.get(message.topic).forEach(receiver => send(receiver, message))`, no roster, no prior mutual handshake), so, on the wire, a forged `{ type: 'announce', from: <fake id> }` published to a room's topic really would reach any already-connected real peer's `SignalingConn`, and that peer's `Room` really would react to it exactly as it reacts to a genuine new peer's arrival (`case 'announce'` in `y-webrtc.js`'s message switch spawns `new WebrtcConn(conn, true, data.from, room)`, which is how real peer discovery works today). Two protocol-level walls remain, both confirmed by reading the client, not the server:

1. **The encryption key gates discovery itself, not just content.** `publishSignalingMessage` (`y-webrtc.js`) AES-encrypts the `announce`/`signal` payload whenever the room has a key, and the receiving `SignalingConn`'s `'message'` handler only attempts to decrypt (and therefore only acts on) a message when `typeof m.data === 'string'`, so a plaintext probe from a key-less prober is silently dropped by every real peer. This app's rooms are encrypted by default (§5) and meeting-link rooms always derive a key (§6.2), so the probe's defining property (works without the room key, because it's a signaling-layer trick, not a content-layer one) does not hold for the WebRTC transport: for encrypted rooms it is not that the probe is expensive, it is that it cannot function at all without the key, which would make it a key-holding probe, a different and much weaker thing than what this section promises.
2. **The only way to provoke a reply is not passive.** The hub probe never sends a byte; it only reads what the server pushes unprompted. On WebRTC there is no unprompted push: the prober must publish a forged `announce`, and that message makes a real peer's `Room` construct a genuine `WebrtcConn`: a real `RTCPeerConnection` that begins real ICE/STUN gathering on that peer's own network, and that shows up in that peer's own UI as a phantom "Reaching" connection (`reachingCount()` in `webrtc.ts` counts any `webrtcConns` entry whose data channel hasn't opened yet, exactly the state a probe-provoked `WebrtcConn` is in). The prober can tear down its own side the instant it sees a reply, but has no way to cancel what it caused on the real peer's side; that phantom connection only clears on the peer's own ICE timeout. This is a cost imposed on an unwitting third party's device and network, not a server the operator chose to run: worse than the hub's already-documented `Y.Doc` leak, and it breaks this section's own "peek at the handshake, never perturb" design, which the hub probe genuinely achieves (it sends nothing) and this WebRTC variant cannot.

Both walls are protocol-level, not a missing feature to build around: (1) is inherent to how `y-webrtc` encrypts its signaling channel, and (2) is inherent to there being no server-side roster to read: the pub/sub relay confirmed above means the *only* way to learn who is subscribed is to make a subscriber react, and reacting is never free on that subscriber's side. `MeetingJoinDialog`'s probe call is skipped whenever the deployment is on the WebRTC transport (`hallUrl` is `undefined`).
