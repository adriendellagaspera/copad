...
- **`Import…`** (`Editor.svelte`'s `importFile()`) — the read side's counterpart to Export: a one-shot local file, picked via `src/format/filePicker.ts` (`pickFile()`, shared with `src/storage/local.ts`'s own picker), dispatched to the matching `Codec` by extension and decoded straight into the shared `Y.Doc`. Unlike export, this is a write — `canImport` re-derives the write-gate (`role === SessionRole.Writer && !writeLocked`) independently of ProseMirror's own `editable` check, since `decode()` bypasses it entirely; `importFile()` re-checks the same condition at its own top so the gate holds even called directly, not just via a hidden button. Replacing a non-empty document asks for confirmation first (`window.confirm`), since it overwrites content for every live collaborator.
