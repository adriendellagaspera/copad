...
{#if view}
  <div class="toolbar" role="toolbar" aria-label="Formatting">
    ...
    <button data-active={strike} aria-pressed={strike} onclick={run(commands.strike)} title="Strikethrough" aria-label="Strikethrough"><s>S</s></button>
    <button data-active={code}   aria-pressed={code}   onclick={run(commands.code)}   title="Inline code (Mod+`)" aria-label="Inline code">{'</>'}</button>
    ...
    <button data-active={quote}     aria-pressed={quote}     onclick={run(commands.blockquote)}     title="Blockquote (type > + space)" aria-label="Blockquote">❝</button>
