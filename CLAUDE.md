...
- **localStorage access** — everything persisted (cache pref, theme, storage tokens, backend config, filenames, per-room passwords) goes through `src/localStore.ts`: `readString`/`writeString`/`removeKey`/`readJSON`/`writeJSON`. It's the single place that wraps `localStorage` in try/catch, so a disabled / full / private-mode store degrades to the fallback (reads) or a no-op (writes) instead of throwing — no per-call try/catch anywhere else.
